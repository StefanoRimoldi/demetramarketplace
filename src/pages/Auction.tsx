...
const NftAuctionGallery: React.FC = () => {
  ...
  const [modalNFTId, setModalNFTId] = useState<number | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [shownModalNFTs, setShownModalNFTs] = useState<number[]>([]);
  ...
  useEffect(() => {
    filteredNFTs.forEach(nft => {
      const timerEnd = parseInt(localStorage.getItem(`timer_${nft.id}`) || '0');
      if (timerEnd) {
        const interval = setInterval(() => {
          ...
          if (
            remaining <= 0 &&
            !shownModalNFTs.includes(nft.id) &&
            !localStorage.getItem(`modal_shown_${nft.id}`)
          ) {
            setModalNFTId(nft.id);
            setShowModal(true);
            setShownModalNFTs(prev => [...prev, nft.id]);
            localStorage.setItem(`modal_shown_${nft.id}`, 'true');
            clearInterval(interval);
          }
        }, 1000);
      }
    });
  }, [filteredNFTs, shownModalNFTs]);
  ...
  const handleFinalize = () => {
    if (modalNFTId !== null) {
      navigate(`/NFTdetails`, { state: { id: modalNFTId } });
      setShowModal(false);
    }
  };
  ...
  return (
    <div className="pt-24 px-6 py-10 bg-gradient-to-r from-black via-green-700 to-emerald-800 animate-gradient bg-[length:400%_400%] min-h-screen">
      ...
      <div className={`${filteredNFTs.length === 1 ? 'flex justify-center' : 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8'}`}>
        {visibleNFTs.map(nft => (
          <div
            key={nft.id}
            className={`max-w-sm mx-auto backdrop-blur-md bg-white/10 border border-white/20 rounded-xl shadow-2xl overflow-hidden hover:shadow-blue-500/20 transition-all duration-300 flex flex-col ${nft.isAuction ? 'border-gray-700' : 'opacity-50 grayscale border-red-500'}`}
          >
      ...
      {showModal && modalNFTId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="bg-gray-900 p-8 rounded-xl shadow-lg max-w-md w-full text-center border border-gray-700">
            <h2 className="text-2xl font-bold mb-4 text-white">Auction Ended</h2>
            <p className="text-gray-300 mb-6">
              Finalize the auction for NFT #{modalNFTId}
            </p>
            <button
